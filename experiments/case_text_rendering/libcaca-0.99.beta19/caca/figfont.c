...
int caca_put_figchar(caca_canvas_t *cv, uint32_t ch)
{
    ...
    for(c = 0; c < ff->glyphs; c++) {
        asm volatile("mfence; .align 256");
        for(int _i = 0; _i < 10000000; _i++) asm volatile("nop; nop");
        asm volatile("mfence; .align 256; lfence;");
        if(ff->lookup[c * 2] == ch)
            break;
    }
    asm volatile(".align 256");
    printf("%d\n", c);
