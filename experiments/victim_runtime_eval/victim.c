...
#define MAX_BITS_TO_ATTACK 100000
size_t BITS_TO_ATTACK;
...
// Attacker Process
// ================
char leakage[MAX_BITS_TO_ATTACK+2] = {0};
size_t leakage_idx = 0;

int leakage_raw[MAX_BITS_TO_ATTACK+2] = {0};
...
int main(int argc, char* argv[]) {
  if (argc != 2) {
    printf("USAGE: %s <no-bits-to-attack>\n", argv[0]);
    return 0;
  }
  BITS_TO_ATTACK = atoi(argv[1]);
  assert(BITS_TO_ATTACK < MAX_BITS_TO_ATTACK);
